  ...
  <div class="login" style="position: fixed; top:0px; left:0px;height:100%;width:100%; background-color: #1F2D3D; z-index:100">
    <div class="loginBg">
      ...
      <div class="loginContent">
        ...
        <p class="loginMean"><span> <router-link style="color:rgb(80, 182, 161);text-decoration: none" to="/ForgetPwdPersonal">忘记密码</router-link> </span><span><router-link style="color: rgb(80, 182, 161);
    text-decoration: none;" to="/sign">注册用户</router-link></span></p>
      </div>
      <div ><button
        class="loginBtn"
        @click="Submit"
...
<script>
  export default {
    ...
    methods:{
      Submit(){
        this.$api.post('home/login',this.formData,(res)=>{
          if(res.status == 200){
            if(res.body.status == 1){
              ...
              return false
                ...
                // 到系统管理员页面
//              this.$router.replace({path:'/selfNav',query:{isShowMeau:1}});
              // 登录成功之后保存一个变量，显示菜单
              // 成功之后返回一个标识，是个人还是国网
...
//                    sessionStorage.setItem('isShowMeau', 1);
            }
          }
        })
      }
