...
<template>
  <div class="contract">
    <div class="cMe_title">
      ...
      <span class="title">每日发电量</span>
      <div class="cright">
        <span class="r"></span>
        <div class="right"></div>
      </div>
    </div>
    <span class="r-text" id="xx" @click="faDianLiang">发电量明细查询</span>
    <div class="hello">
...
<script>
  ...
  export default {
    ...
    methods: {
      // 第一次加载请求数据，展示echarts
      getEeacher(obj){
        this.$api.post('productionElectricity/getByMonth',obj,(res)=>{
      ...
      // 发电量明细
      faDianLiang(){
        this.$router.push({path:'/Header/ElectricityDetails',query:
          {isShowMeau:1,searchSign:0}});
      },
      // 获取当前时间日期
      getDate(){
        let Dates = new Date();
